...
import { BadRequestError, ForbiddenError, UnauthorizedError, sharedSecretMatches } from '@visin/backend-core';
...
export const requireApiKey = (req: Request, _res: Response, next: NextFunction): void => {
  ...
  if (!sharedSecretMatches(req.headers['x-internal-api-key'], process.env.FILE_SERVICE_API_KEY)) {
