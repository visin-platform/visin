...
const SERVER_INSTRUCTIONS = [
  ...
  '',
  'A run\'s last epoch is not its result. Training past the point of best validation is ordinary,',
  'and the checkpoint anyone would ship is the best one — so compare runs on where each metric',
  'peaked, which get_training and compare_trainings both give you, rather than on the final row.',
  'A metric that ends far from its best end is a finding in itself, not a worse run.',
  '',
  'You can look at the frames a run rendered. list_visualizations shows what exists — prediction',
  'overlays, ground-truth comparisons, segmentation maps — and view_visualizations returns the',
  'images themselves. Use them when a metric says a model is worse without saying how: a few',
  'frames answer "what is it actually getting wrong" in a way a number cannot. Ask for the few',
  'worth comparing rather than a whole epoch. Dataset images and labelled frames are still not',
  'reachable — only what a training run rendered.',
  '',
  'Conclusions can be written down. Check list_findings before analysing something from scratch —',
  'the answer may already be recorded — and use record_finding when you work out something worth',
