...
describe('testResultService.aggregateTestResults', () => {
  ...
  it('computes mean and std for a single test result', () => {
    ...
    expect(result.rain.car.iou).toEqual({ mean: 0.8, std: 0 });
    expect(result.rain.overall.mIoU_foreground).toEqual({ mean: 0.5, std: 0 });
  });
  ...
  it('averages a metric across multiple test results and computes population std dev', () => {
    ...
    expect(result.day.car.iou.mean).toBeCloseTo(0.8);
    expect(result.day.car.iou.std).toBeCloseTo(0.2);
  });
  ...
  it('ignores non-numeric and NaN metric values instead of propagating NaN', () => {
    ...
    expect(result.day.car.iou).toEqual({ mean: 0.5, std: 0 });
  });
  ...
  it('aggregates inference_time across conditions using avg_per_sample_ms', () => {
    ...
    expect(result.inference_time.avg_per_sample_ms.mean).toBeCloseTo(15);
  });
  ...
  it('handles a class present in only some test results (missing values simply do not contribute)', () => {
    ...
    expect(result.day.car.iou.mean).toBeCloseTo(0.5);
    expect(result.day.truck.iou.mean).toBeCloseTo(0.8);
    expect(result.day.truck.iou.std).toBeCloseTo(0);
  });
});
