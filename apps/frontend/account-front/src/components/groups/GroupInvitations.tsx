...
export default function GroupInvitations({ groupId, canInviteOwner }: { groupId: string; canInviteOwner: boolean }) {
  ...
  const create = useMutation({
    mutationFn: () => groupService.createInvitation(groupId, role === 'owner' && !canInviteOwner ? 'member' : role),
  ...
  const revoke = useMutation({
    mutationFn: () => groupService.revokeInvitations(groupId),
    onSuccess: () => setLink('')
  });
  ...
  return (
    <Box>
      <Typography variant="body2" sx={{ mb: 2 }}>
        Share a single-use invitation directly with the person you want to add. Links expire after seven days.
      </Typography>
      <Box sx={{ display: 'flex', gap: 2 }}>
        <TextField
          ...
          value={role === 'owner' && !canInviteOwner ? 'member' : role}
          onChange={(event) => setRole(event.target.value as GroupRole)}
          disabled={busy}
          sx={{ minWidth: 150 }}
        >
          {(canInviteOwner ? ['owner', 'admin', 'member'] : ['admin', 'member']).map((value) => (
        ...
        <Button color="error" disabled={busy} onClick={() => revoke.mutate()}>
