...
const ClassIoUOverEpochsChart: React.FC<ClassIoUOverEpochsChartProps> = ({ 
  ...
  if (!regularSeries.some(s => s.data.some(v => v !== null)) && !twoDSeries.some(s => s.data.some(v => v !== null))) {
    return null;
  }
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      {regularSeries.some(s => s.data.some(v => v !== null)) && (
      ...
      {/* 2D Classes Chart */}
      {twoDSeries.some(s => s.data.some(v => v !== null)) && (
        <Paper sx={{ p: 3, position: 'relative' }}>
          <Box sx={{ width: '100%', height: 400 }}>
            <LineChart
              xAxis={[{ data: epochNumbers, label: 'Epoch' }]}
