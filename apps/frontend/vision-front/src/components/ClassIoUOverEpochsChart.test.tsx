...
import ClassIoUOverEpochsChart from './ClassIoUOverEpochsChart';
...
describe('ClassIoUOverEpochsChart', () => {
  it('renders nothing when epochs is empty', () => {
    const { container } = render(<ClassIoUOverEpochsChart epochs={[]} />);
    expect(container.firstChild).toBeNull();
  });

  it('renders nothing when no class has iou data', () => {
    const epochs = [makeEpoch(1, { train: { loss: 0.1 }, val: { loss: 0.2 } })];
    const { container } = render(<ClassIoUOverEpochsChart epochs={epochs} />);
    expect(container.firstChild).toBeNull();
  });

  it('renders a chart for regular classes with iou data, labeled "<class> IoU"', () => {
    const epochs = [
      makeEpoch(1, { val: { loss: 0.1, vehicle: { iou: 0.5 } as any } }),
      makeEpoch(2, { val: { loss: 0.1, vehicle: { iou: 0.7 } as any } })
    ];
    render(<ClassIoUOverEpochsChart epochs={epochs} />);
    expect(screen.getByText('vehicle IoU')).toBeInTheDocument();
  });
  ...
  it('splits out _2d suffixed classes into a separate chart', () => {
    ...
    render(<ClassIoUOverEpochsChart epochs={epochs} />);
    expect(screen.getByText('vehicle IoU')).toBeInTheDocument();
    expect(screen.getByText('vehicle_2d IoU')).toBeInTheDocument();
  });
