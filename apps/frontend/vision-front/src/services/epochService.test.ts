...
describe('epochService', () => {
  ...
  it('createEpoch posts epoch data', async () => {
    ...
    await epochService.createEpoch({ epoch: 1 } as any);
    expect(mockedApi.post).toHaveBeenCalledWith('/epochs', { epoch: 1 });
  });

  it('uploadEpoch merges trainingId into payload when provided', async () => {
    mockedApi.post.mockResolvedValue({ data: { success: true, data: { _id: 'e1' } } });
    await epochService.uploadEpoch({ epoch: 1 }, 't1');
    expect(mockedApi.post).toHaveBeenCalledWith('/epochs/upload', { epoch: 1, trainingId: 't1' });
  });

  it('uploadEpoch sends payload unmodified without trainingId', async () => {
    mockedApi.post.mockResolvedValue({ data: { success: true, data: { _id: 'e1' } } });
    await epochService.uploadEpoch({ epoch: 1 });
    expect(mockedApi.post).toHaveBeenCalledWith('/epochs/upload', { epoch: 1 });
  });

  it('createEpochsBatch wraps epochs array', async () => {
    mockedApi.post.mockResolvedValue({ data: { success: true, data: [] } });
    await epochService.createEpochsBatch([{ epoch: 1 } as any]);
    expect(mockedApi.post).toHaveBeenCalledWith('/epochs/batch', { epochs: [{ epoch: 1 }] });
  });

  it('updateEpoch puts partial data', async () => {
    mockedApi.put.mockResolvedValue({ data: { success: true, data: {} } });
    await epochService.updateEpoch('e1', { epoch: 2 } as any);
  ...
  describe('uploadOrUpdateEpoch', () => {
    it('updates when payload has an _id', async () => {
      mockedApi.put.mockResolvedValue({ data: { success: true, data: {} } });
      await epochService.uploadOrUpdateEpoch({ _id: 'e1', epoch: 3 }, 't1');
      expect(mockedApi.put).toHaveBeenCalledWith('/epochs/e1', { _id: 'e1', epoch: 3, trainingId: 't1' });
    });

    it('updates when payload has an epoch_uuid', async () => {
      mockedApi.put.mockResolvedValue({ data: { success: true, data: {} } });
      await epochService.uploadOrUpdateEpoch({ epoch_uuid: 'uuid-1', epoch: 3 });
      expect(mockedApi.put).toHaveBeenCalledWith('/epochs/uuid-1', { epoch_uuid: 'uuid-1', epoch: 3 });
    });
    ...
    it('falls back to uploadEpoch when update fails', async () => {
      ...
      mockedApi.post.mockResolvedValue({ data: { success: true, data: {} } });
      await epochService.uploadOrUpdateEpoch({ _id: 'e1', epoch: 3 }, 't1');
      expect(mockedApi.post).toHaveBeenCalledWith('/epochs/upload', { _id: 'e1', epoch: 3, trainingId: 't1' });
    });

    it('creates a new epoch when payload has no id', async () => {
      mockedApi.post.mockResolvedValue({ data: { success: true, data: {} } });
      await epochService.uploadOrUpdateEpoch({ epoch: 3 }, 't1');
      expect(mockedApi.post).toHaveBeenCalledWith('/epochs/upload', { epoch: 3, trainingId: 't1' });
