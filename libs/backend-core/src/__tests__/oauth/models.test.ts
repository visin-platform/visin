...
describe('OAuth models', () => {
  it('are each registered once, so a second import does not overwrite them', () => {
    ...
    expect(mongoose.models.OAuthGrant).toBe(OAuthGrant);
  ...
  it('keep each kind in its own collection', () => {
    ...
    expect(OAuthGrant.collection.name).toBe('oauth_grants');
  ...
  it('requires history to identify the original grant generation', async () => {
    await expect(new RefreshToken({}).validate()).rejects.toMatchObject({
      errors: { tokenHash: expect.anything(), clientId: expect.anything(), grantId: expect.anything(), generation: expect.anything() }
    });
  });

  it('requires grant authority and defaults to no scopes', async () => {
    const grant = new OAuthGrant({});
    expect(grant.scopes).toEqual([]);
    await expect(grant.validate()).rejects.toMatchObject({ errors: {
      _id: expect.anything(), clientId: expect.anything(), userId: expect.anything(),
      generation: expect.anything(), currentTokenHash: expect.anything(), resource: expect.anything(), grantedAt: expect.anything()
    } });
  });
